...
FMT_CONSTEXPR auto lossless_integral_conversion (
    const From from, int& ec
) -> To
{
  ec = 0;
...
template <
    typename To, typename From,
    FMT_ENABLE_IF (!std::is_same<From, To>::value)>
FMT_CONSTEXPR auto safe_float_conversion (
    const From from, int& ec
) -> To
{
  ec = 0;
  ...
  static_assert (
      std::is_floating_point<To>::value, "To must be floating"
  );
...
) -> To
{
  ...
  struct Factor
      : std::ratio_divide<
            typename From::period, typename To::period> {};
  ...
  using IntermediateRep = typename std::common_type<
      typename From::rep, typename To::rep,
  ...
  if FMT_CONSTEXPR20 (Factor::num != 1) {
    constexpr auto max1 =
        detail::max_value<IntermediateRep>() /
  ...
  if FMT_CONSTEXPR20 (Factor::den != 1) {
    using common_t = typename std::common_type<
        IntermediateRep, intmax_t>::type;
...
template <typename Duration>
using utc_time =
    std::chrono::time_point<detail::utc_clock, Duration>;

template <class Duration>
using local_time =
    std::chrono::time_point<detail::local_t, Duration>;
...
class formatbuf : public StreamBuf {
 private:
  ...
  using streamsize =
      decltype (std::declval<StreamBuf>().sputn (nullptr, 0));
 ...
 protected:
  ...
  auto xsputn (const char_type* s, streamsize count)
      -> streamsize override
  {
...
template <typename CodeUnit>
void write_codecvt (
    codecvt_result<CodeUnit>& out, string_view in,
    const std::locale& loc
)
{
  FMT_PRAGMA_CLANG (diagnostic push)
  FMT_PRAGMA_CLANG (diagnostic ignored "-Wdeprecated")
  auto& f = std::use_facet<
      std::codecvt<CodeUnit, char, std::mbstate_t>> (loc);
  FMT_PRAGMA_CLANG (diagnostic pop)
  auto mb = std::mbstate_t();
  const char* from_next = nullptr;
  auto result = f.in (
      mb, in.begin(), in.end(), from_next, std::begin (out.buf),
...
auto write_encoded_tm_str (
    OutputIt out, string_view in, const std::locale& loc
) -> OutputIt
{
  if (detail::use_utf8 && loc != get_classic_locale()) {
    ...
    if (!u.convert (
            {unit.buf, to_unsigned (unit.end - unit.buf)}
        )) {
      FMT_THROW (format_error ("failed to format time"));
    }
...
auto write_tm_str (
    OutputIt out, string_view sv, const std::locale& loc
) -> OutputIt
{
...
inline void do_write (
    buffer<Char>& buf, const std::tm& time,
    const std::locale& loc, char format, char modifier
)
{
  auto&& format_buf =
      formatbuf<std::basic_streambuf<Char>> (buf);
  auto&& os = std::basic_ostream<Char> (&format_buf);
  os.imbue (loc);
  const auto& facet = std::use_facet<std::time_put<Char>> (loc);
  auto end =
      facet.put (os, os, Char (' '), &time, format, modifier);
  if (end.failed()) {
    FMT_THROW (format_error ("failed to format time"));
  }
}
...
) -> OutputIt
{
  ...
  do_write<char> (buf, time, loc, format, modifier);
  return write_encoded_tm_str (
      out, string_view (buf.data(), buf.size()), loc
  );
}
...
) -> To
{
  ...
  using factor =
      std::ratio_divide<FromPeriod, typename To::period>;

  using common_rep = typename std::common_type<
      FromRep, typename To::rep, decltype (factor::num)>::type;
  common_rep count =
      from.count();  // This conversion is lossless.
  ...
  if FMT_CONSTEXPR20 (factor::num != 1) {
    ...
    const auto min =
        (std::numeric_limits<common_rep>::min)() / factor::num;
...
template <
    typename To, typename FromRep, typename FromPeriod,
    FMT_ENABLE_IF (!is_similar_arithmetic_type<
                   FromRep, typename To::rep>::value)>
auto duration_cast (
    std::chrono::duration<FromRep, FromPeriod> from
) -> To
{
...
auto to_time_t (sys_time<Duration> time_point) -> std::time_t
{
  ...
  return detail::duration_cast<
             std::chrono::duration<std::time_t>> (
...
{
  ullong digits =
      a | (b << 24) | (static_cast<ullong> (c) << 48);
...
template <typename OutputIt>
auto write_padding (OutputIt out, pad_type pad, int width)
    -> OutputIt
{
  ...
  return detail::fill_n (
      out, width, pad == pad_type::space ? ' ' : '0'
  );
}
    ...
    switch (c) {
      ...
      case 'e':
        handler.on_day_of_month (
            numeric_system::standard, pad_type::space
        );
        break;
      // Hour, minute, second:
      case 'H':
...
struct null_chrono_spec_handler {
  ...
  FMT_CONSTEXPR void on_dec0_week_of_year (
      numeric_system, pad_type
  )
  {
    unsupported();
  }
  FMT_CONSTEXPR void on_dec1_week_of_year (
      numeric_system, pad_type
  )
  {
    unsupported();
  }
  FMT_CONSTEXPR void on_iso_week_of_year (
      numeric_system, pad_type
  )
  {
    unsupported();
...
void write_fractional_seconds (
    OutputIt& out, Duration d, int precision = -1
)
{
  constexpr auto num_fractional_digits = count_fractional_digits<
      Duration::period::num, Duration::period::den>::value;
  ...
  const auto fractional =
      d - detail::duration_cast<std::chrono::seconds> (d);
  const auto subseconds =
      std::chrono::treat_as_floating_point<
          typename subsecond_precision::rep>::value
          ? fractional.count()
          : detail::duration_cast<subsecond_precision> (
                fractional
            )
                .count();
  auto n =
      static_cast<uint32_or_64_or_128_t<long long>> (subseconds);
...
)
{
  ...
  if (num_fractional_digits < 0) {
    ...
    using namespace std;
    num_fractional_digits = count_fractional_digits<
        Duration::period::num, Duration::period::den>::value;
    if (num_fractional_digits < 6 &&
        static_cast<rep> (round (val)) != val) {
...
class tm_writer {
 private:
  ...
  auto iso_year_weeks (long long curr_year) const noexcept -> int
  {
    ...
    auto curr_p = (curr_year + curr_year / 4 - curr_year / 100 +
                   curr_year / 400) %
                  days_per_week;
    auto prev_p = (prev_year + prev_year / 4 - prev_year / 100 +
                   prev_year / 400) %
                  days_per_week;
    return 52 + ((curr_p == 4 || prev_p == 3) ? 1 : 0);
  }
  auto iso_week_num (int tm_yday, int tm_wday) const noexcept
      -> int
  {
    return (tm_yday + 11 -
            (tm_wday == 0 ? days_per_week : tm_wday)) /
           days_per_week;
  }
 ...
 public:
  tm_writer (
      const std::locale& loc, OutputIt out, const std::tm& tm,
  ...
  void on_datetime (numeric_system ns)
  {
    if (is_classic_) {
      ...
      *out_++ = ' ';
      ...
      format_localized (
          'c', ns == numeric_system::standard ? '\0' : 'E'
      );
    }
  }
  void on_loc_date (numeric_system ns)
  {
    if (is_classic_) {
      ...
      format_localized (
          'x', ns == numeric_system::standard ? '\0' : 'E'
      );
    }
  }
  ...
  void on_us_date()
  {
    ...
    write_digit2_separated (
        buf, to_unsigned (tm_mon() + 1), to_unsigned (tm_mday()),
  ...
  void on_iso_date()
  {
    ...
    write_digit2_separated (
        buf + 2, static_cast<unsigned> (year % 100),
        to_unsigned (tm_mon() + 1), to_unsigned (tm_mday()), '-'
    );
    out_ = copy<Char> (
        std::begin (buf) + offset, std::end (buf), out_
    );
  }
  ...
  void on_12_hour_time()
  {
    if (is_classic_) {
      char buf[8];
      write_digit2_separated (
          buf, to_unsigned (tm_hour12()), to_unsigned (tm_min()),
          to_unsigned (tm_sec()), ':'
      );
      out_ = copy<Char> (std::begin (buf), std::end (buf), out_);
      *out_++ = ' ';
...
template <
    typename T, FMT_ENABLE_IF (std::is_floating_point<T>::value)>
inline auto mod (T x, int y) -> T
{
...
inline auto get_milliseconds (
    std::chrono::duration<Rep, Period> d
) -> std::chrono::duration<Rep, std::milli>
{
  // This may overflow and/or the result may not fit in the target type.
#if FMT_SAFE_DURATION_CAST
  using common_seconds_type = typename std::common_type<
      decltype (d), std::chrono::seconds>::type;
  auto d_as_common =
      detail::duration_cast<common_seconds_type> (d);
  ...
  auto ms = detail::duration_cast<
      std::chrono::duration<Rep, std::milli>> (diff);
  return ms;
#else
  auto s = detail::duration_cast<std::chrono::seconds> (d);
  return detail::duration_cast<std::chrono::milliseconds> (
      d - s
  );
#endif
}
...
template <
    ...
    FMT_ENABLE_IF (std::is_integral<Rep>::value)>
auto format_duration_value (OutputIt out, Rep val, int)
    -> OutputIt
{
...
auto format_duration_value (OutputIt out, Rep val, int precision)
    -> OutputIt
{
  auto specs = format_specs();
  specs.precision = precision;
  specs.set_type (
      precision >= 0 ? presentation_type::fixed
...
template <typename OutputIt>
auto copy_unit (string_view unit, OutputIt out, wchar_t)
    -> OutputIt
{
...
struct duration_formatter {
  ...
  duration_formatter (
      iterator o, std::chrono::duration<Rep, Period> d,
      locale_ref loc
  )
      : out (o),
  ...
  void write (
      Rep value, int width, pad_type pad = pad_type::zero
  )
  {
    write_sign();
    if (isnan (value)) {
    ...
    if (ns == numeric_system::standard) {
      if (std::is_floating_point<rep>::value) {
        ...
        write_floating_seconds (
            buf, std::chrono::duration<rep, Period> (val),
            precision
        );
        if (negative) {
          *out++ = '-';
...
class year_month_day {
 ...
 public:
  ...
  constexpr year_month_day (
      const year& y, const month& m, const day& d
  ) noexcept
...
struct formatter<weekday, Char>
 ...
 public:
      ...
      -> const Char*
  {
    ...
    return use_tm_formatter_
               ? formatter<std::tm, Char>::parse (ctx)
               : it;
  }

  template <typename FormatContext>
  auto format (weekday wd, FormatContext& ctx) const
      -> decltype (ctx.out())
  {
    auto time = std::tm();
...
struct formatter<day, Char> : private formatter<std::tm, Char> {
 ...
 public:
  ...
  template <typename FormatContext>
  auto format (day d, FormatContext& ctx) const
      -> decltype (ctx.out())
  {
    auto time = std::tm();
    ...
    w.on_day_of_month (
        detail::numeric_system::standard, detail::pad_type::zero
    );
    return w.out();
  }
};
...
struct formatter<month, Char>
 ...
 public:
  ...
  template <typename FormatContext>
  auto format (month m, FormatContext& ctx) const
      -> decltype (ctx.out())
  {
    auto time = std::tm();
    time.tm_mon =
        static_cast<int> (static_cast<unsigned> (m)) - 1;
    if (use_tm_formatter_) {
      return formatter<std::tm, Char>::format (time, ctx);
    }
    detail::get_locale loc (this->localized(), ctx.locale());
...
struct formatter<year_month_day, Char>
 ...
 public:
      ...
      -> decltype (ctx.out())
  {
    ...
    time.tm_mon =
        static_cast<int> (static_cast<unsigned> (val.month())) -
        1;
    time.tm_mday =
        static_cast<int> (static_cast<unsigned> (val.day()));
    if (use_tm_formatter_) {
      return formatter<std::tm, Char>::format (time, ctx);
    }
...
struct formatter<std::chrono::duration<Rep, Period>, Char> {
 ...
 public:
      ...
      -> const Char*
  {
    ...
    if ((c >= '0' && c <= '9') || c == '{') {
      it =
          detail::parse_width (it, end, specs_, width_ref_, ctx);
      if (it == end) {
        return it;
      }
    }
    ...
    if (*it == '.') {
      ...
      it = detail::parse_precision (
          it, end, specs_, precision_ref_, ctx
      );
    }
    if (it != end && *it == 'L') {
  ...
  template <typename FormatContext>
  auto format (
      std::chrono::duration<Rep, Period> d, FormatContext& ctx
  ) const -> decltype (ctx.out())
  {
    ...
    detail::handle_dynamic_spec (
        specs.dynamic_width(), specs.width, width_ref_, ctx
    );
    detail::handle_dynamic_spec (
        specs.dynamic_precision(), precision, precision_ref_, ctx
    );
    if (begin == end || *begin == '}') {
      out = detail::format_duration_value<Char> (
          out, d.count(), precision
      );
...
struct formatter<std::tm, Char> {
 ...
 protected:
  ...
  FMT_CONSTEXPR auto do_parse (
      parse_context<Char>& ctx, bool has_timezone
  ) -> const Char*
  {
    auto it = ctx.begin(), end = ctx.end();
    if (it == end || *it == '}') {
  ...
  ) const -> decltype (ctx.out())
  {
    ...
    auto loc_ref =
        specs.localized() ? ctx.locale() : locale_ref();
    detail::get_locale loc (
        static_cast<bool> (loc_ref), loc_ref
    );
    auto w =
        detail::tm_writer<basic_appender<Char>, Char, Duration> (
            loc, out, tm, subsecs
        );
...
struct formatter<sys_time<Duration>, Char>
      ...
      -> decltype (ctx.out())
  {
    ...
    Duration subsecs = detail::duration_cast<Duration> (
        epoch -
        detail::duration_cast<std::chrono::seconds> (epoch)
    );
    if (subsecs.count() < 0) {
      auto second = detail::duration_cast<Duration> (
...
struct formatter<local_time<Duration>, Char>
  ...
  template <typename FormatContext>
  auto format (
      local_time<Duration> val, FormatContext& ctx
  ) const -> decltype (ctx.out())
  {
    auto time_since_epoch = val.time_since_epoch();
    auto seconds_since_epoch =
        detail::duration_cast<std::chrono::seconds> (
            time_since_epoch
        );
