...
#define FMT_USE_CONSTEVAL \
  0  // consteval is broken in Apple clang < 14.
...
#if FMT_HAS_CPP_ATTRIBUTE(noreturn) && !FMT_MSC_VERSION && \
    !defined(__NVCC__)
...
constexpr auto is_constant_evaluated (
...
{
  FMT_ASSERT (
      std::is_unsigned<Int>::value || value >= 0,
      "negative value"
  );
...
struct is_std_string_like<
    T, void_t<decltype (std::declval<T>().find_first_of (
...
constexpr FMT_ALWAYS_INLINE auto narrow (const char* s) -> const
    char*
{
  return s;
}
...
struct is_back_insert_iterator<
    It, bool_constant<std::is_same<
            decltype (adl::invoke_back_inserter<
                      typename It::container_type>()),
...
class basic_string_view {
 ...
 public:
  ...
  {
#if FMT_HAS_BUILTIN(__builtin_strlen) || FMT_GCC_VERSION || \
    FMT_CLANG_VERSION
    if (std::is_same<Char, char>::value &&
        !detail::is_constant_evaluated()) {
      size_ = __builtin_strlen (
          detail::narrow (s)
  ...
  FMT_CONSTEXPR auto starts_with (
  ...
  FMT_CONSTEXPR auto compare (basic_string_view other) const
      -> int
  {
    int cmp = detail::compare (
        data_, other.data_, min_of (size_, other.size_)
    );
    if (cmp != 0) {
      return cmp;
    }
  ...
  FMT_CONSTEXPR friend auto operator== (
      basic_string_view lhs, basic_string_view rhs
  ) -> bool
  {
...
template <typename T>
struct is_contiguous : detail::is_contiguous<T> {};
...
template <
    typename T, typename Char = char, typename Enable = void>
struct formatter {
...
class basic_specs {
 ...
 public:
  ...
  {
    data_ =
        (data_ & ~align_mask) | (unsigned (a) << align_shift);
  }
  ...
  {
    data_ =
        (data_ & ~width_mask) | (unsigned (w) << width_shift);
  }
  ...
  constexpr auto fill_unit() const -> Char
  {
    ...
    return Char (
        uchar (fill_data_[0]) | uchar (fill_data_[1]) << 8 |
  ...
  FMT_CONSTEXPR void set_fill (basic_string_view<Char> s)
  {
    ...
    if (size == 1) {
      unsigned uchar =
          static_cast<detail::unsigned_char<Char>> (s[0]);
...
class parse_context {
 ...
 public:
  ...
  FMT_CONSTEXPR auto next_arg_id() -> int
  {
    if (next_arg_id_ < 0) {
      report_error (
          "cannot switch from manual to automatic argument "
          "indexing"
      );
      return 0;
    }
  ...
  FMT_CONSTEXPR void check_arg_id (int id)
  {
    if (next_arg_id_ > 0) {
      report_error (
          "cannot switch from automatic to manual argument "
          "indexing"
      );
      return;
    }
...
class locale_ref {
 ...
 public:
  ...
  template <
      typename Locale,
      FMT_ENABLE_IF (sizeof (Locale::collate) != 0)>
...
constexpr auto to_string_view (const Char* s)
    -> basic_string_view<Char>
{
  return s;
}
template <
    typename T, FMT_ENABLE_IF (is_std_string_like<T>::value)>
...
struct named_arg : view {
  ...
  static_assert (
      !is_named_arg<T>::value, "nested named arguments"
  );
};
...
void init_named_arg (
    named_arg_info<Char>*, int& arg_index, int&, const T&
)
{
  ++arg_index;
}
template <
    ...
    FMT_ENABLE_IF (is_named_arg<T>::value)>
void init_named_arg (
    named_arg_info<Char>* named_args, int& arg_index,
    int& named_arg_index, const T& arg
)
{
  check_for_duplicate<Char> (
      named_args, named_arg_index, arg.name
  );
...
FMT_CONSTEXPR void init_static_named_arg (
    named_arg_info<Char>*, int& arg_index, int&
)
{
  ++arg_index;
}
...
template <typename T>
using format_as_result = remove_cvref_t<
    decltype (format_as (std::declval<const T&>()))>;
template <typename T>
using format_as_member_result =
    remove_cvref_t<decltype (formatter<T>::format_as (
        std::declval<const T&>()
    ))>;
...
struct use_format_as<
    T, bool_constant<
           std::is_arithmetic<format_as_result<T>>::value>>
    : std::true_type {};
template <typename T>
struct use_format_as_member<
    T, bool_constant<std::is_arithmetic<
           format_as_member_result<T>>::value>>
    : std::true_type {};
...
template <
    typename Char, typename T, typename U = remove_const_t<T>>
auto has_formatter_impl (
    T* p, buffered_context<Char>* ctx = nullptr
...
{
  return decltype (has_formatter_impl<Char> (
...
template <typename T, typename Char>
using mapped_t = decltype (detail::type_mapper<Char>::map (
    std::declval<T&>()
));
...
using mapped_type_constant =
    type_constant<mapped_t<T, Char>, Char>;

template <
    typename T, typename Context,
    type TYPE = mapped_type_constant<
...
class compile_parse_context : public parse_context<Char> {
 ...
 public:
  constexpr explicit compile_parse_context (
      basic_string_view<Char> fmt, int num_args,
...
template <
    typename Char, FMT_ENABLE_IF (std::is_integral<Char>::value)>
...
FMT_CONSTEXPR auto parse_dynamic_spec (
    const Char* begin, const Char* end, int& value,
...
) -> const Char*
{
  auto result = parse_dynamic_spec (
      begin, end, specs.width, width_ref, ctx
  );
...
) -> const Char*
{
  ...
  if (end - begin > 1) {
    ...
    c = parse_align (next) == align::none ? to_ascii (*begin)
                                          : '\0';
  }
  else {
    if (begin == end) {
  ...
  constexpr auto integral_set =
      sint_set | uint_set | bool_set | char_set;
  struct {
    const Char*& begin;
    format_specs& specs;
    type arg_type;
  ...
  for (;;) {
    switch (c) {
      ...
      case '-':
        enter_state (
            state::sign, in (arg_type, sint_set | float_set)
        );
        ++begin;
        break;
      case '#':
    ...
    case '6': case '7': case '8': case '9': case '{':
        ...
        begin = parse_width (
            begin, end, specs, specs.width_ref, ctx
        );
        break;
      case '.':
        enter_state (
      ...
      case 'L':
        enter_state (
            state::locale, is_arithmetic_type (arg_type)
        );
        specs.set_localized();
        ++begin;
        break;
...
FMT_CONSTEXPR void parse_format_string (
    basic_string_view<Char> fmt, Handler&& handler
)
{
...
FMT_CONSTEXPR inline auto check_char_specs (
...
) -> bool
{
  ...
  if (specs.align() == align::numeric ||
      specs.sign() != sign::none || specs.alt()) {
...
FMT_CONSTEXPR auto invoke_parse (parse_context<Char>& ctx)
    -> const Char*
{
  ...
  using formatted_type =
      conditional_t<formattable, mapped_type, int>;
...
template <
    typename Char, int NUM_ARGS, int NUM_NAMED_ARGS,
    bool DYNAMIC_NAMES>
class format_string_checker {
 private:
  type types_[max_of<size_t> (1, NUM_ARGS)];
  named_arg_info<Char>
      named_args_[max_of<size_t> (1, NUM_NAMED_ARGS)];
  compile_parse_context<Char> context_;

  using parse_func = auto (*) (parse_context<Char>&)
      -> const Char*;
  parse_func parse_funcs_[max_of<size_t> (1, NUM_ARGS)];

 public:
  template <typename... T>
  FMT_CONSTEXPR explicit format_string_checker (
  ...
  FMT_CONSTEXPR void on_replacement_field (
      int id, const Char* begin
  )
  {
    on_format_specs (
        id, begin, begin
  ...
  FMT_CONSTEXPR auto on_format_specs (
...
class buffer {
 ...
 protected:
  ...
  constexpr buffer (
      grow_fun grow, T* p = nullptr, size_t sz = 0,
      size_t cap = 0
  ) noexcept
  ...
  FMT_CONSTEXPR void set (
      T* buf_data, size_t buf_capacity
  ) noexcept
  {
    ptr_ = buf_data;
 ...
 public:
  ...
  {
    static_assert (
        std::is_same<T, U>() || std::is_same<U, char>(), ""
    );
    while (begin != end) {
      auto size = size_;
...
class container_buffer
 ...
 private:
  ...
  static FMT_CONSTEXPR void grow (
      buffer<value_type>& buf, size_t capacity
  )
  {
...
class iterator_buffer<
    ...
    enable_if_t<
        is_back_insert_iterator<OutputIt>::value &&
            is_contiguous<
                typename OutputIt::container_type>::value,
        typename OutputIt::container_type::value_type>>
    : public container_buffer<
          typename OutputIt::container_type> {
 private:
  using base =
      container_buffer<typename OutputIt::container_type>;
...
struct custom_value {
  ...
  void* value;
  void (*format) (
      void* arg, parse_context<char_type>& parse_ctx,
      Context& ctx
  );
};
...
class value {
 public:
  ...
  constexpr FMT_INLINE value (long x FMT_BUILTIN)
      : value (long_type (x))
  {
  }
  ...
  constexpr FMT_INLINE value (ullong x FMT_BUILTIN)
      : ulong_long_value (x)
  {
  }
  FMT_INLINE value (native_int128 x FMT_BUILTIN)
      : int128_value (x)
  {
  }
  FMT_INLINE value (native_uint128 x FMT_BUILTIN)
      : uint128_value (x)
  {
  }
  ...
  constexpr FMT_INLINE value (const void* x FMT_BUILTIN)
      : pointer (x)
  {
  }
  ...
  template <
      typename T,
      FMT_ENABLE_IF (
          use_formatter<T>::value || !FMT_BUILTIN_TYPES
      )>
 ...
 private:
  template <
      typename T, FMT_ENABLE_IF (has_formatter<T, char_type>())>
...
{
  return NUM_ARGS <= max_packed_args
             ? encode_types<Context, T...>()
             : is_unpacked_bit | NUM_ARGS;
}

template <typename Context, int NUM_ARGS>
using arg_t = conditional_t<
    NUM_ARGS <= max_packed_args, value<Context>,
...
struct named_arg_store {
  ...
  {
    int arg_index = 0, named_arg_index = 0;
    FMT_APPLY_VARIADIC (init_named_arg (
        named_args, arg_index, named_arg_index, values
    ));
  }
  ...
  auto operator= (const named_arg_store& rhs)
      -> named_arg_store& = delete;
  auto operator= (named_arg_store&& rhs)
      -> named_arg_store& = delete;
...
template <bool B>
constexpr bool enforce_compile_checks()
{
...
constexpr auto is_locking() -> bool
{
  return locking<remove_cvref_t<T1>>::value ||
         is_locking<T2, Tail...>();
}
...
#if FMT_WIN32
FMT_API void vprint_mojibake (
    FILE*, string_view, format_args, bool
);
#else  // format_args is passed by reference since it is defined later.
inline void vprint_mojibake (
    FILE*, string_view, const format_args&, bool
)
{
}
#endif
...
template <typename Char>
FMT_CONSTEXPR void parse_context<Char>::do_check_arg_id (
    int arg_id
)
{
  ...
  if (detail::is_constant_evaluated() && use_constexpr_cast) {
    auto ctx =
        static_cast<detail::compile_parse_context<Char>*> (this);
...
template <typename Char>
FMT_CONSTEXPR void parse_context<Char>::check_dynamic_spec (
    int arg_id
)
{
  using detail::compile_parse_context;
  if (detail::is_constant_evaluated() && use_constexpr_cast) {
    static_cast<compile_parse_context<Char>*> (this)
...
class basic_appender {
 ...
 public:
  ...
  constexpr basic_appender (detail::buffer<T>& buf)
      : container (&buf)
  {
  }
...
class basic_format_arg {
 ...
 public:
  ...
  basic_format_arg (
      const detail::named_arg_info<char_type>* args, size_t size
  )
      : value_ (args, size)
  {
  }
...
class basic_format_args {
 private:
  ...
  using store = detail::format_arg_store<
      Context, NUM_ARGS, NUM_NAMED_ARGS, DESC>;
 ...
 public:
  ...
  template <
      int NUM_ARGS, int NUM_NAMED_ARGS, ullong DESC,
  ...
  constexpr FMT_ALWAYS_INLINE basic_format_args (
      ...
      : desc_ (
            DESC |
            (NUM_NAMED_ARGS != 0 ? +detail::has_named_args_bit
                                 : 0)
        ),
        values_ (s.args)
  {
  ...
  FMT_CONSTEXPR auto get (int id) const -> format_arg
  {
    auto arg = format_arg();
    if (!is_packed()) {
      if (unsigned (id) < unsigned (max_size())) {
        arg = args_[id];
      }
      return arg;
    }
  ...
  template <typename Char>
  FMT_CONSTEXPR auto get_id (basic_string_view<Char> name) const
      -> int
  {
    if (!has_named_args()) {
      return -1;
    ...
    const auto& named_args =
        (is_packed() ? values_[-1] : args_[-1].value_)
            .named_args;
  ...
  {
    return int (
        is_packed() ? ullong (detail::max_packed_args)
...
class context {
 ...
 public:
  ...
  constexpr context (
      iterator out, format_args args, locale_ref loc = {}
  )
...
struct fstring {
 ...
 public:
      ...
      : str (s, N - 1)
  {
    ...
    constexpr bool unused =
        detail::enforce_compile_checks<sizeof (s) != 0>();
    (void)unused;
  }
  template <
      typename S,
  ...
  {
    if (FMT_USE_CONSTEVAL) {
      detail::parse_format_string<char> (
          str, checker (str, arg_pack())
      );
    }
    constexpr bool unused =
        detail::enforce_compile_checks<sizeof (s) != 0>();
...
using is_formattable = bool_constant<!std::is_same<
    detail::mapped_t<
        conditional_t<std::is_void<T>::value, int*, T>, Char>,
    void>::value>;
#if defined(__cpp_concepts) && __cpp_concepts >= 201907L
template <typename T, typename Char = char>
concept formattable =
    is_formattable<remove_reference_t<T>, Char>::value;
#endif
...
struct formatter<
    ...
    : detail::native_formatter<
          T, Char, detail::type_constant<T, Char>::value> {};
...
template <
    typename Context = context, typename... T,
...
auto vformat_to (
    OutputIt&& out, string_view fmt, format_args args
...
FMT_INLINE auto format_to (
    OutputIt&& out, format_string<T...> fmt, T&&... args
) -> remove_cvref_t<OutputIt>
{
...
auto vformat_to_n (
    OutputIt out, size_t n, string_view fmt, format_args args
) -> format_to_n_result<OutputIt>
{
  using traits = detail::fixed_buffer_traits;
  auto buf =
      detail::iterator_buffer<OutputIt, char, traits> (out, n);
  detail::vformat_to (buf, fmt, args, {});
...
template <size_t N>
FMT_DEPRECATED auto vformat_to (
...
{
  auto result =
      vformat_to_n (out, N, fmt.str, vargs<T...>{{args...}});
  return {result.out, result.size > N};
}
...
FMT_API void vprintln (
    FILE* f, string_view fmt, format_args args
);
FMT_API void vprint_buffered (
    FILE* f, string_view fmt, format_args args
);
...
FMT_INLINE void print (format_string<T...> fmt, T&&... args)
{
  ...
  detail::is_locking<T...>()
      ? vprint_buffered (stdout, fmt.str, va)
      : vprint (fmt.str, va);
}
 ...
 */
template <typename... T>
FMT_INLINE void print (
    FILE* f, format_string<T...> fmt, T&&... args
)
{
  vargs<T...> va = {{args...}};
  if FMT_CONSTEXPR20 (!detail::use_utf8) {
