...
int main (
    ...
    plog::init (ctx.debug_mode ? plog::debug : plog::none, &consoleAppender);
    ...
    if (bcf_hdr_sync (ohdr.get()) < 0) {
        throw std::runtime_error ("failed to sync VCF output header");
    }
    if (bcf_hdr_write (ovcf.get(), ohdr.get()) < 0) {
        throw std::runtime_error ("failed to write VCF output header");
    };
    ...
    auto write_record = [&] () { return bcf_write (ovcf.get(), ohdr.get(), b1.get()); };
    while (bcf_read (vcffh.get(), vcf_hdr.get(), b1.get()) == 0) {

        if (!var::check_record_filters(b1.get(), vcf_hdr.get(), {ctx.flt_inc, ctx.flt_exc})) {
            if (write_record() != 0) { goto ERR_WRITE; } else continue;
        }
        ...
        for (auto &n : norms) {
            auto pm = pileup_analyse (n.first.get(), n.second.get(), b1.get(), ctx.flag_inc, ctx.flag_exc, vcf_hdr.get());
        ...
        if (!checked_read_len && sample_total_pileup.read_lengths.size() >= 5) {
            ...
            const double deviation = std::abs (median_len - static_cast<double> (ctx.exp_read_len))
        ...
        if (n_obs_qpos < 2 || qpos_pwd) {
            ...
            auto stat_fn = [&] (const auto &v) {
                ...
                return spatial::ripley_k(
                        *pwds,
                        search_radius,
                        static_cast<double>(v.size()) / static_cast<double> (ctx.exp_read_len)
                    );
            };
            ...
            if (ctx.uniform_sim) {
                ...
                std::uniform_int_distribution<uint64_t> qpos_gen{0, ctx.exp_read_len - 1}; // inclusive ends
        ...
        if (write_record() != 0) { goto ERR_WRITE; };
