...
int pileup_func (void* data, bam1_t* b)
{
  ...
  while (true) {
    ...
    flag = b->core.flag;
    if (((flag & d->exc_flag) == 0) &&
        ((flag & d->inc_flag) == d->inc_flag)) {
...
static PileupColumn get_pileup (
...
static double get_alignment_score_from_tag (
    const bam_pileup1_t* p
)
{
  auto *const qname = bam_get_qname (p->b);
...
static PileupMetrics get_metrics_paired_end (
...
)
{
  ...
  for (const auto *const p : pc) {
    auto *const qname = bam_get_qname (p->b);

    auto *const raw_mc = bam_aux_get (p->b, "MC");
    if (raw_mc == NULL) {
      throw std::runtime_error (
    ...
    out.read_lengths.push_back (
        static_cast<uint32_t> (p->b->core.l_qseq)
    );

        // don't double count templates
    if (qnames.contains (qname)) {     // qname already seen
      continue;
    }
    qnames.insert (qname);

    if (p->b->core.tid != p->b->core.mtid ||
        ((p->b->core.flag & BAM_FMUNMAP) != 0)) {
      continue;
    }
    ...
    out.template_endpoints.emplace_back (
        as_uint (*tco.first), as_uint (*tco.second)
    );
  }
  bam_destroy1 (mateb);
  return out;
...
)
{
  auto *aln_iter = sam_itr_queryi (
      aln_idx, var->rid, var->pos, var->pos + var->rlen
  );
  if (aln_iter == nullptr) {
    throw std::runtime_error (
        std::format (
            ...
            "{}:{}-{}",
            bcf_hdr_id2name (vcf_hdr, var->rid), var->pos + 1,
            var->pos + 1 + var->rlen
        )
    );
  }
  PileupCapture pfc{
      aln_fh, aln_iter, sam_flag_include, sam_flag_exclude
  };
...
static PileupColumn filter (
    const PileupColumn& pc,
    const std::function<bool (const bam_pileup1_t*)>&
        predicate_fn
)
{
  PileupColumn out;
...
)
{
  ...
  auto pile_supporting =
      filter (pile_all, [&support_fn, var] (const auto p) {
        return support_fn (var, p);
      });
...
{
  a.query_position.insert (
      end (a.query_position), begin (b.query_position),
      end (b.query_position)
  );
  a.normalised_as.insert (
      end (a.normalised_as), begin (b.normalised_as),
      end (b.normalised_as)
  );
  a.template_endpoints.insert (
      end (a.template_endpoints), begin (b.template_endpoints),
      end (b.template_endpoints)
  );
  a.read_lengths.insert (
      end (a.read_lengths), begin (b.read_lengths),
      end (b.read_lengths)
  );
  return a;
}
