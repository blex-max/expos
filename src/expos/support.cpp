...
static int get_var_indel_len (const bcf1_t* m)
{
  const std::string ref = m->d.allele[0];
  const std::string alt = m->d.allele[1];

  // negative for del, pos for ins
  return static_cast<int> (alt.length()) -
...
bool eval_support_ins (const bcf1_t* m, const bam_pileup1_t* p)
{
  ...
  const std::string alt = m->d.allele[1];
  const auto var_ins =
      alt.substr (1, static_cast<size_t> (bam_ins_len));
  auto ins_match = strcmp (ks_c_str (&ins), var_ins.c_str()) ==
                   0;  // check bases match
