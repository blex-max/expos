...
{
  std::vector<bool> out;
  out.reserve (flt.size());
  for (const auto& f : flt) {
    out.push_back (
        bcf_has_filter (
            hdr, const_cast<bcf1_t*> (rec),
...
{
  const auto iflt = bcf_has_filters (hdr, b, f.must_have);
  ...
  const auto eflt = bcf_has_filters (hdr, b, f.must_not_have);
...
{
  const auto *const rid_name = bcf_hdr_id2name (h, v->rid);
  return std::format (
      "{}:{}-{}", rid_name, v->pos + 1, v->pos + 1 + v->rlen
  );
}
