...
//     alndv other;
// };
using PileupColumn = std::vector<bam_pileup1_t>;
inline PileupColumn get_pileup (
    htsFile   *aln_fh,
    hts_idx_t *aln_idx,
    bcf1_t    *var,
    int        sam_flag_include,
    ...
    // SEGFAULT HERE. GOES OUT OF SCOPE, DIES
    // Maybe I can copy? or return or something
    bam_plp_upt buf{
        bam_plp_init (pileup_func, &pfc),
        bam_plp_destroy
    };     // initialize pileup
    ...
    while (
        ...
        if (plp_pos != var->pos) {
            ...
            for (size_t i = 0; i < static_cast<size_t> (n_plp); i++) {
                pv.push_back(*(plarr + i));  // copy into vector TODO propagate
            }
        }
        break;
    }
...
inline PileupColumn get_supporting (const PileupColumn &pc, const bcf1_t *var, int mutation_type) {
    PileupColumn out;
    for (const auto p : pc) {
...
inline std::pair<PileupMetrics, PileupMetrics> pileup_partition_and_anaylse (
    ...
    auto allp = get_pileup(aln_fh, aln_idx, var, sam_flag_include, sam_flag_exclude);
    auto supportingp = get_supporting(allp, var, mutation_type);

    // we analyse both independently for overlap reasons
    auto supporting_metrics = get_metrics(supportingp);
    auto all_metrics = get_metrics(allp);
    return out;
}
