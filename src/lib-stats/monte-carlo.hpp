...
template <
    typename StatT, typename StatFn, typename DrawFn,
    typename EffFn,
    typename ObsT =
        typename std::invoke_result_t<DrawFn&>::value_type>
  requires std::same_as<
               std::invoke_result_t<DrawFn&>,
               std::vector<ObsT>> &&
           std::same_as<
               std::invoke_result_t<
                   StatFn&, const std::vector<ObsT>&>,
               StatT> &&
           std::same_as<
               std::invoke_result_t<
                   EffFn&, StatT, const std::vector<StatT>&>,
               std::optional<double>>
inline stat_eval_s sim_to_bg (
    StatT ev_stat, DrawFn&& drawfn, StatFn&& statfn,
    EffFn&& efffn, size_t nsim = 2500
)
{
  stat_eval_s res;
  assert (nsim > 0);
...
inline std::vector<T> subsample_wo_replace (
    const std::vector<T>& obs, size_t n, std::mt19937& rng
)
{
  std::vector<T> out;
