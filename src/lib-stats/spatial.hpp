...
struct line_seg {
  ...
  line_seg() = delete;
  line_seg (uint64_t a, uint64_t b)
  {
    lmost = a > b ? b : a;
    rmost = a > b ? a : b;
  }
...
class PairMatrix {
 private:
  ...
  PairMatrix (std::vector<uint64_t> v, size_t dim)
      : mat (v), dim_ (dim)
  {
    assert (dim > 1);
 ...
 public:
  ...
  static std::optional<PairMatrix> from_sample (
      const std::vector<U>& obs, F&& sym_pairfn
  )
  {
    assert (!obs.empty());
...
inline double ripley_k (
    const PairMatrix& pwd, uint64_t t, double point_intensity
)
{
  ...
  const auto mean_lt =
      static_cast<double> (lt_sum) / static_cast<double> (n);
