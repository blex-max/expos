...
inline double entropy_lz76 (std::string_view s)
{
  ...
  while (prefix_len + comp_len <= nchar) {
    ...
    else  // mismatch
    {
      ...
      ++i;
      if (i == prefix_len) {  // memory completely searched
        ++lz_cmplx;
...
*/
inline std::vector<size_t> periodic_rle (
    const std::string& s, size_t max_k
)
{
