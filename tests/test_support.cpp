...
struct ReadHelper {
  ...
  ReadHelper (const char* seq, int32_t qpos, int indel = 0)
  {
    ...
    auto l = strlen (seq);
    uint32_t cig =
        bam_cigar_gen (static_cast<int> (l), BAM_CMATCH);
    bam_set1 (
        b, 1, "r", 0, 0, 0, 0, 1, &cig, 0, 0, 0, l, seq, nullptr,
        0
    );
    pl = {};
    pl.b = b;
...
struct IndelReadHelper {
  ...
  bam_pileup1_t pl;
  IndelReadHelper (
      const char* seq, int l_seq, const uint32_t* cigar,
      int n_cigar, int32_t qpos, int indel = 0
  )
  {
    b = bam_init1();
    bam_set1 (
        b, 1, "r", 0, 0, 0, 0, static_cast<size_t> (n_cigar),
        cigar, 0, 0, 0, static_cast<size_t> (l_seq), seq,
        nullptr, 0
    );
    pl = {};
    pl.b = b;
...
TEST_CASE ("eval_support_ins")
{
  ...
  SECTION (
      "insertion bases mismatch (correct length, wrong bases)"
  )
  {
