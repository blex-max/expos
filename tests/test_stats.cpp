...
std::size_t brute_pairs_1d (const std::vector<int32_t>& v, uint64_t r)
{
  std::size_t count = 0;
  for (std::size_t i = 0; i < v.size(); ++i) {
    for (std::size_t j = i + 1; j < v.size(); ++j) {
      const uint64_t d = v[i] > v[j] ? static_cast<uint64_t> (v[i] - v[j])
...
{
  REQUIRE_FALSE (percentile (std::vector<uint32_t>{}, 0.5).has_value());
  REQUIRE (*percentile (std::vector<uint32_t>{7}, 0.5) == Approx (7.0));
  ...
  REQUIRE (
      *percentile (std::vector<uint32_t>{1, 2, 3, 4}, 0.5) == Approx (2.5)
  );
  // linear interpolation: rank 0.25 between 0 and 10 -> 2.5
  REQUIRE (
      *percentile (std::vector<uint32_t>{0, 10}, 0.25) == Approx (2.5)
  );
  // floating-point sample + duplicates
  REQUIRE (
      *percentile (std::vector<double>{5.0, 5.0, 5.0, 5.0}, 0.25) ==
      Approx (5.0)
  );
}
...
TEST_CASE ("count_pairs_within_1d")
{
  ...
  REQUIRE (count_pairs_within_1d ({7, 7, 7}, 1) == 3);  // dups, dist 0 < 1
  REQUIRE (count_pairs_within_1d ({1, 2, 3}, 0) == 0);  // radius 0 -> none
...
TEST_CASE ("count_pairs_within (generic)")
{
  ...
  auto manhattan = [&] (const Point& a, const Point& b) -> uint64_t {
    return absDiff (a.first, b.first) + absDiff (a.second, b.second);
  };
  const std::vector<Point> pts{{0, 0}, {1, 1}, {10, 10}};
  REQUIRE (count_pairs_within (pts, 3, manhattan) == 1);  // (0,0)-(1,1)=2
  REQUIRE (count_pairs_within (pts, 2, manhattan) == 0);  // 2 not < 2
  REQUIRE (count_pairs_within (pts, 100, manhattan) == 3);
  REQUIRE (count_pairs_within (std::vector<Point>{}, 5, manhattan) == 0);
}
...
TEST_CASE ("ripley_k")
{
  ...
  const double intensity = static_cast<double> (obs.size()) / 100.0;
...
TEST_CASE ("monte_carlo_pvalue")
{
  ...
  SECTION ("zero-variance null yields no effect size")
  {
    ...
    const auto res = monte_carlo_pvalue (3.0, draw, identity, 100);
  ...
  SECTION ("deterministic under a fixed seed; p-value in (0,1]")
  {
    ...
    auto run = [&] (uint32_t seed) {
      ...
      auto draw = [&]() { return subsample_wo_replace (pop, 20, rng); };
      auto stat = [] (const std::vector<int32_t>& s) {
        return static_cast<double> (count_pairs_within_1d (s, 5));
      };
    ...
    REQUIRE (a.effectSize.has_value() == b.effectSize.has_value());
...
TEST_CASE ("subsample_wo_replace")
{
  ...
  SECTION ("size, membership, no replacement")
  {
    ...
    REQUIRE (uniq.size() == 3);
    for (const int x : s) {
      REQUIRE (std::find (obs.begin(), obs.end(), x) != obs.end());
    }
  }
  ...
  {
    std::mt19937 rng (1);
    const auto full = subsample_wo_replace (obs, obs.size(), rng);
